...
Next.js는 프론트와 백엔드가 한 프로젝트 안에 있다. `app/api/` 폴더가 Express의 `routes/` 역할을 한다.
...
`app/api/auth/[...nextauth]/` 이 폴더는 NextAuth가 내부적으로 사용하는 경로다.

`...`는 "catch-all" 라우트. `/api/auth/signin`, `/api/auth/signout`, `/api/auth/session` 등 NextAuth가 필요한 모든 경로를 이 하나의 폴더가 처리한다. 직접 코드를 작성하지 않아도 NextAuth가 알아서 동작한다.
